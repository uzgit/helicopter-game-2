...
class Population(object):
    ...
    def init_run(self, n=None):

        if self.config.no_fitness_termination and (n is None):
            raise RuntimeError("Cannot have no generational limit with no fitness termination")

        self.generations_to_run = n
    ...
    def finish_run(self):
        ...
        print([genome.fitness for genome in itervalues(self.population)])
        ...
        if self.generation_number < self.generations_to_run:
            #return list(iteritems(self.population)), self.config
            return True
        else:
